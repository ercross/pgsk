...
import 'payment_details_tab.dart';
...
class ChoosePaymentTab extends StatefulWidget {
  ...
  ChoosePaymentTab({@required this.allowedHeight, @required this.allowedWidth});
...
class _ChoosePaymentTabState extends State<ChoosePaymentTab> {
  List<PaymentOption> _fetchPaymentOptions() {
    ...
    return <PaymentOption>[
      PaymentOption(
          imageUrl: urlPrefix + "mastercard_logo.png", name: "MasterCard"),
  ...
  void _buildMiddleContent() {
    if (currentPage == 0)
      setState(() => _middleContent = _buildChoosePaymentOption(
          _PaymentOptionsRadio(_fetchPaymentOptions())));

    if (currentPage == 1)
      setState(() => _middleContent = _buildPaymentDetailsForm());
  }

  Widget _buildChoosePaymentOption(_PaymentOptionsRadio paymentOptionRadio) {
    return SizedBox(
      child: paymentOptionRadio,
    );
  }
    ...
    return Container(
        ...
        child: ChosenPaymentOption.paymentOption == null
            ? Text("Choose Payment Method",
                textAlign: TextAlign.center, style: PGSK.homepageTexts)
            : Row(
                ...
                children: [
                  ...
                  Image.asset(ChosenPaymentOption.paymentOption.imageUrl,
                      height: 70, width: 70)
                ],
              ));
  }
...
class PaymentOption {
  ...
  const PaymentOption({@required this.imageUrl, @required this.name});
}
...
class _PaymentOptionsRadioState extends State<_PaymentOptionsRadio> {
  ...
  Widget _buildPaymentOption(
      {@required int index, @required PaymentOption paymentOption}) {
    return Row(
  ...
  Widget _buildPaymentOptions() {
    ...
    List<Row> paymentOptionsW = [];

    for (int i = 0; i < widget.paymentOptions.length; i++) {
      paymentOptionsW.add(_buildPaymentOption(
          index: i, paymentOption: widget.paymentOptions[i]));
    }
