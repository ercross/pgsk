...
class Cart with ChangeNotifier {
  ...
  List<ProductAndQuantity> _productsAndQuantity = [];
  double _total = 0;
  ...
  void addProduct(Product product) {
    if (_uniqueProducts.contains(product)) {
      productsAndQuantity
          .firstWhere((productQuantity) =>
              productQuantity.product.name.compareTo(product.name) == 0)
          .quantity++;
    } else {
  ...
  void fetchMyCart() async {
    if (_totalNumberOfProducts == 0 &&
        _total == 0 &&
        _productsAndQuantity.isEmpty) {
